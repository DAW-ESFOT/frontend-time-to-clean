...
const TableComplaints = () => {
  ...
  const { data, error, mutate } = useSWR(
    `/complaints?page=${page + 1}`,
    fetcher
  );
  const { data: dataFilter, error: errorFilter, mutate: mutateFilter } = useSWR(
    `/complaints${filter}`,
    fetcher
  );
                ...
                <Table aria-label="customized table">
                  <TableHead>
                    <TableRow>
                      ...
                      <StyledTableCell align="center" style={{ width: 350 }}>
                        Queja
                      </StyledTableCell>
                  ...
                  <TableBody>
                    {data.data.map((Complaint) => (
                      <StyledTableRow key={Complaint.id}>
                        <StyledTableCell align="center">
                          {Complaint.created_at.substr(0, 10)}
                          <br />
                        ...
                        <StyledTableCell align="left">
                          Barrio: {Complaint.neighborhood_name}
                          <br />
                          Camión: {Complaint.truck.license_plate}
                          <br />
                        ...
                        <StyledTableCell align="left">
                          <Tooltip title="Editar" placement="bottom">
                            <IconButton
                              onClick={() =>
                                handleOpenEditComplaint(Complaint.id)
                              }
                              color="secondary"
