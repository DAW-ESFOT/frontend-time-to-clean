    ...
    <>
      <div>
        {neighborhood ? (
                ...
                <div>
                  <Typography component={"span"} color={"secondary"}>
                    <Box display="flex" justifyContent="center" m={1} p={1}>
                      <h1>Barrio {neighborhood.name}</h1>
                    </Box>
                  </Typography>
                  <div>
          ...
          <div>No se ha podido cargar la información</div>
        )}
      </div>
    </>
  );
...
export async function getStaticProps(context) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/neighborhoods/${barriosId}`
  );
...
export async function getStaticPaths() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/neighborhoods`
  );
  const data = await res.json();
